...
func (gc *mcGroupCreator) create(ctx context.Context, groups []string, ignoreExist bool) error {
	...
	for _, group := range groups {
		...
		if err != nil {
			if alreadyExist(err) {
				if !ignoreExist {
					errors = append(errors, fmt.Sprintf("group: '%v' already exists, use the --ignore-existing-groups flag if you would like to update it with the MConnect label.", group))
					continue
				} else {
			...
			errors = append(errors, fmt.Sprintf("group: '%v', err: %v", group, err))
			continue
		}
	...
	if errors != nil {
		return fmt.Errorf("%s", multipleErrorsMessage(errors))
	}
	return nil
}

func multipleErrorsMessage(errors []string) string {
	errMessage := ""
